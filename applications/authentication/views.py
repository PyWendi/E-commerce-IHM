...
class NotificationViewset(viewsets.ModelViewSet):
    ...
    def marked_as_seen(self, request, pk):
        """
        Need only the `notifacitonId` to operate and return a `status 200`
        """
...
class ContactViewSet(viewsets.ModelViewSet):
    ...
    def trigger_channel(self, room_name, message_data):
        channel = get_channel_layer()
        async_to_sync(channel.group_send)(room_name, message_data)
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            self.trigger_channel("admin_notification", dict(type="admin_notification", operation=True))
